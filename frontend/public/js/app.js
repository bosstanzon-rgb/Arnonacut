...
(function () {
  ...
  function renderResultsKitState() {
    ...
    if (state.kitUnlocked && state.kitUrls) {
      ...
      $("results-kit-title-locked").textContent = t("results.kitLockedTitle");
      $("results-kit-sub-locked").textContent = t("results.kitLockedSub");
      const cta = $("results-btn-checkout");
      if (cta) {
        cta.textContent = t("results.kitCheckout", { price: window.ArnonaI18n.formatIls(KIT_PRICE) });
  ...
  function wire() {
    ...
    $("btn-resume")?.addEventListener("click", () => {
      ...
      if (state.view === "quiz") {
        ...
        void refreshPremiumFromServer().then(() => renderResultsKitState());
